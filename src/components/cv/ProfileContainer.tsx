...
export function ProfileContainer({ cv }: ProfileContainerProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="bg-[#0f0e0c] border border-red-500/30 text-red-400 hover:bg-red-500/10 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
          >
            Delete Profile
          </button>
          <button
            onClick={() => setIsReuploading(true)}
            className="bg-[#1a1916] border border-[#2d2a26] text-white hover:bg-[#2d2a26]/50 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="bg-[#0f0e0c] border border-[#2d2a26] rounded-xl p-6 sm:p-8 flex flex-col md:flex-row md:items-center gap-6 md:gap-12 relative overflow-hidden">
        ...
        <div className="flex-1 space-y-2 relative z-10">
          <div className="text-amber-400 font-semibold tracking-wider text-xs uppercase mb-1">
            Current Role
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          {/* ── Experience ────────────────────────────────────────────────────────── */}
          <section className="bg-[#1a1916] border border-[#2d2a26] rounded-xl p-6 space-y-6">
            <div className="flex items-center gap-2 text-white pb-4 border-b border-[#2d2a26]">
            ...
            <div className="space-y-6">
              {data.experience.map((exp, i) => (
                <div key={i} className="relative pl-6 before:absolute before:left-1.5 before:top-2 before:bottom-0 before:w-[1px] before:bg-[#2d2a26] last:before:hidden py-1">
                  <div className="absolute left-0 top-2 w-3 h-3 bg-[#0f0e0c] border-2 border-blue-500 rounded-full" />
                  <div className="mb-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                    ...
                    <span className="text-amber-400 text-sm font-medium">{exp.duration}</span>
                  </div>
          ...
          <section className="bg-[#1a1916] border border-[#2d2a26] rounded-xl p-6 space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {data.education.map((edu, i) => (
                <div key={i} className="bg-[#0f0e0c] border border-[#2d2a26]/50 rounded-lg p-4">
        ...
        <div className="space-y-8 lg:col-span-1">
          ...
          <section className="bg-[#1a1916] border border-[#2d2a26] rounded-xl p-6 space-y-6">
            ...
            <p className="text-xs text-gray-500 pt-2 border-t border-[#2d2a26]/50">
